    ...
    for participant in os.listdir(root):
        ...
        for symbol in os.listdir(participant_dir):
            ...
            for img in os.listdir(symbol_dir):
                ...
                imgray = (255 - (cv2.filter2D(gray, -1, kernel_aya)))
                #imgray = (255-((cv2.GaussianBlur(gray, (5, 5), 0))))

                #imgray1 = deskew(imgray)
                ...
                #Image = cv2.convertScaleAbs(dst, alpha=2, beta=0)





                rv=0.03 #was 0.03 for 97.1
                rh=0.03
                height = dst.shape[0]
                width = dst.shape[1]
                ...
                #Im_padded = cv2.copyMakeBorder(blackAndWhiteImage,60,60,20,20, cv2.BORDER_CONSTANT, None, value=0)

                res = cv2.resize(Im_padded, dsize=(dim, dim), interpolation=cv2.INTER_CUBIC)
                #res=cv.bilateralFilter(res,3,70,70)
